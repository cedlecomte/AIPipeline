...
const copy2clipboard = require("@vavt/copy2clipboard");
...
const CDN_IDS = {
  hljs: `${config.prefix}-hljs`,
  hlcss: `${config.prefix}-hlCss`,
  prettier: `${config.prefix}-prettier`,
  prettierMD: `${config.prefix}-prettierMD`,
  cropperjs: `${config.prefix}-cropper`,
  croppercss: `${config.prefix}-cropperCss`,
  screenfull: `${config.prefix}-screenfull`,
  mermaidM: `${config.prefix}-mermaid-m`,
  mermaid: `${config.prefix}-mermaid`,
  katexjs: `${config.prefix}-katex`,
  katexcss: `${config.prefix}-katexCss`
};
const userZoom = (props, html) => {
  const editorId = vue.inject("editorId");
  const { noImgZoomIn } = props;
  const zoomHander = util.debounce(() => {
...
const useHighlight = (props) => {
  ...
  const hljsRef = vue.shallowRef(config.globalConfig.editorExtensions.highlight.instance);
  vue.onMounted(() => {
  ...
  vue.watch(
    () => highlight.value.css,
    () => {
      if (props.noHighlight || config.globalConfig.editorExtensions.highlight.instance) {
        return;
      }
...
const useMermaid = (props) => {
  ...
  const rootRef = vue.inject("rootRef");
  const { editorExtensions, editorExtensionsAttrs, mermaidConfig } = config.globalConfig;
...
const useKatex = (props) => {
  const katex = vue.shallowRef(config.globalConfig.editorExtensions.katex.instance);
  vue.onMounted(() => {
    var _a, _b;
...
const MermaidPlugin = (md, options) => {
  ...
  md.renderer.rules.fence = (tokens, idx, ops, env, slf) => {
    ...
    if (token.info === "mermaid") {
      token.attrSet("class", `${config.prefix}-mermaid`);
  ...
  const katexInline = (tokens, idx, options, env, slf) => {
    ...
    if (katexRef.value) {
      const html = katexRef.value.renderToString(
        token.content,
        config.globalConfig.katexConfig({
          throwOnError: false
        })
      );
...
const codetabs = (md, _opts) => {
  ...
  const fenceGroup = (tokens, idx, options, env, slf) => {
    ...
    const collapseTips = `<span class="${config.prefix}-collapse-tips">${dom.StrIcon("collapse-tips", _opts.customIconRef.value)}</span>`;
        ...
        <${tagContainer2} ${slf.renderAttrs(tmpToken2)}>
          <${tagHeader2} class="${config.prefix}-code-head">
            ...
            <div class="${config.prefix}-code-action">
              <span class="${config.prefix}-code-lang">${md.utils.escapeHtml(tokens[idx].info.trim())}</span>
    ...
    for (let i = idx; i < tokens.length; i++) {
      ...
      langs += `
        <input
          ...
          ${checked}
          role="presentation">
        <span class=${config.prefix}-code-lang role="note">${md.utils.escapeHtml(getLangName(token))}</span>`;
    }
    return `
...
const useMarkdownIt = (props, previewOnly) => {
  const { editorConfig, markdownItConfig, markdownItPlugins, editorExtensions } = config.globalConfig;
  const editorId = vue.inject("editorId");
  ...
  md.set({
    highlight: (str, language, attrs) => {
      ...
      const codeSpan = showCodeRowNumber ? index.generateCodeRowNumber(
        codeHtml.replace(/^\n+|\n+$/g, ""),
        str.replace(/^\n+|\n+$/g, "")
  ...
  initLineNumber(md);
  const key = vue.ref(`_article-key_${util.randomId()}`);
  const html = vue.ref(
    props.sanitize(
    ...
    vue.nextTick(() => {
      replaceMermaid().then(() => {
        ...
        if ((_a = editorExtensions.mermaid) == null ? void 0 : _a.enableZoom) {
          clearMermaidEvents();
          clearMermaidEvents = dom.zoomMermaid(
            (_b = rootRef.value) == null ? void 0 : _b.querySelectorAll(
              `#${editorId} p.${config.prefix}-mermaid:not([data-closed=false])`
            ),
            {
  ...
  let timer = -1;
  vue.watch([vue.toRef(props, "modelValue"), needReRender, reRenderRef, languageRef], () => {
  ...
  vue.onMounted(() => {
    ...
    eventName.bus.on(editorId, {
      ...
      callback: () => {
        key.value = `_article-key_${util.randomId()}`;
        markHtml();
      }
    });
  });
...
const UpdateOnDemand = /* @__PURE__ */ vue.defineComponent({
  name: "UpdateOnDemand",
  props: {
    html: {
      type: String,
  ...
  setup(props) {
    ...
    const updateHtmlContent = (updates, deletes) => {
      ...
      updates.forEach(({
        ...
        var _a, _b, _c;
        const targetNode = (_a = htmlContainer.value) == null ? void 0 : _a.childNodes[index2];
        if (!targetNode) {
...
const ContentPreview = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(props) {
    ...
    return () => {
      ...
      }, [vue.createVNode(UpdateOnDemand, {
        "key": key.value,
        "html": html.value
...
const useProvidePreview = (props, rootRef) => {
  const hljsUrls = config.globalConfig.editorExtensions.highlight;
  const hljsAttrs = config.globalConfig.editorExtensionsAttrs.highlight;
  ...
  const usedLanguageText = vue.computed(() => {
    const allText = {
      ...
      ...config.globalConfig.editorConfig.languageUserDefined
    };
...
const useEditorId = (props) => {
  const defaultId = vue.useId();
  return props.id || props.editorId || `${config.prefix}-${defaultId}`;
};
