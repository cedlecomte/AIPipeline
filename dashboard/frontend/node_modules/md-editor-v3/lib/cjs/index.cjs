  ...
  setup(props2, ctx) {
    return () => {
      ...
      return vue.createVNode("div", {
        "class": [`${config$1.prefix}-footer-item`, props2.disabled && `${config$1.prefix}-disabled`],
        "onClick": (e) => {
          var _a;
...
const clearSideEffects = () => {
  Object.keys(MdPreview.CDN_IDS).forEach((key) => {
    const ele = document.getElementById(MdPreview.CDN_IDS[key]);
    if (ele) {
      ele.remove();
    }
  });
...
exports.prefix = config$1.prefix;
...
exports.clearSideEffects = clearSideEffects;
