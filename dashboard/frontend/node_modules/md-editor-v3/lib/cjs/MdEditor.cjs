...
const handlePrettier = async (codeMirrorUt, params) => {
  var _a, _b, _c;
  const prettier = window.prettier || ((_a = config.globalConfig.editorExtensions.prettier) == null ? void 0 : _a.prettierInstance);
  const prettierPlugins = [
    ((_b = window.prettierPlugins) == null ? void 0 : _b.markdown) || ((_c = config.globalConfig.editorExtensions.prettier) == null ? void 0 : _c.parserMarkdownInstance)
  ];
...
const handleMermaid = (type) => {
  const mermaidTemplates = {
    ...
    journey: "journey\n  title My Journey",
    ...config.globalConfig.editorConfig.mermaidTemplate
  };
  return {
    text: `
\`\`\`mermaid
...
const useCodeMirror = (props2) => {
  ...
  const getExtensions = () => {
    ...
    return config.globalConfig.codeMirrorExtensions(
      theme.value,
      extensions,
      getDefaultKeymaps(),
      { editorId }
  ...
  vue.onMounted(() => {
    ...
    eventName.bus.on(editorId, {
      ...
      callback: (lineNumber, value) => {
        ...
        view2.dispatch(
          view2.state.update({ changes: { from: line.from, to: line.to, insert: value } })
        );
      }
    });
    eventName.bus.on(editorId, {
  ...
  return {
    ...
    resetHistory() {
      ...
      (_b = codeMirrorUt.value) == null ? void 0 : _b.view.dispatch({ effects: historyComp.reconfigure(commands.history()) });
    }
  };
};
